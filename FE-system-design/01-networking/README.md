...
## HTTPS Request Lifecycle Explained

This diagram visually represents the **sequence of network interactions** that occur when a user accesses a secure website (`https://`). It includes the **DNS Lookup**, **TCP Handshake**, **SSL/TLS Handshake**, and **HTTP Communication** phases.
...
* The browser queries the **DNS server** to resolve the domain name (e.g., `example.com`) into an **IP address** (e.g., `93.184.216.34`).
* Once resolved, the browser can connect directly to that IP.

### 2. TCP Handshake

* Establishes a **reliable connection** between the client and the web server.
* It follows a **three-way handshake** process:

  1. Client → SYN
  2. Server → SYN-ACK
  3. Client → ACK
* Ensures both parties are ready to send and receive data.

### 3. SSL/TLS Handshake

* Sets up a **secure channel** for encrypted communication.
* The client and server exchange certificates and cryptographic keys.
* It includes:

  * ClientHello → ServerHello
  * Certificate exchange
  * Key negotiation
  * Session encryption establishment

### 4. HTTP Request / Response

* Once the secure connection is established, the client sends an **HTTP GET** request.
* The server responds with data in chunks (14KB, 28KB, 56KB, etc.), followed by acknowledgments (`ACK`) for reliable transmission.
* This ensures **data integrity** and **complete delivery**.

---

### 🔐 Summary of Data Flow

| Phase         | Purpose                   | Protocol |
| ------------- | ------------------------- | -------- |
| DNS Lookup    | Domain name resolution    | DNS      |
| TCP Handshake | Reliable connection setup | TCP      |
| SSL Handshake | Secure connection setup   | TLS/SSL  |
| HTTP Request  | Data transfer             | HTTPS    |
